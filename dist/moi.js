...
let selfCardJson = null;
export function getSelfCardJson() {
    return selfCardJson;
}
/**
 ...
 * .url / .skills[].tags).
 */
export function stashSelfCard(opts) {
 ...
 * non-numeric ids.
 */
export function newestAgentId(ids) {
    if (!ids.length)
        return null;
    const num = (s) => {
        const m = /(\d+)\s*$/.exec(s);
 ...
 */
export async function getMyCurrentAgentId(creds) {
...
async function openRegistry(creds, cardUrlRef) {
...
export async function registerOnMoi(opts) {
    ...
    const cardUrlRef = { url: base ? `${base}/moi/card.json` : undefined };
    ...
    const agentId = await registry.createAgent({ protocol: "a2a", protocolVersion: "1.0" }, {
        name: opts.displayName,
        description: opts.bio,
        version: "0.2.0",
        url: base,
...
export async function discoverDatingAgents(creds, opts = {}) {
    const { registry } = await openRegistry(creds);
    const mine = new Set(await registry.getMyAgents());
    const ids = await registry.getAllAgentIds();
    const allow = new Set((opts.peerOwners || []).map(normAddr).filter(Boolean));
    const matches = [];
    for (const id of ids) {
        if (mine.has(id))
            continue; // skip our own agents
...
export async function getMyAgentIds(creds) {
    const { registry } = await openRegistry(creds);
    return (await registry.getMyAgents());
}
/**
 ...
 */
export async function getMyActiveAgentIds(creds) {
    const { registry } = await openRegistry(creds);
    const ids = (await registry.getMyAgents()) || [];
    const active = [];
    for (const id of ids) {
 ...
 */
export async function deprecateMyAgents(creds, agentId) {
    const { registry } = await openRegistry(creds);
    let targets;
    if (agentId) {
        targets = [agentId];
...
export async function getMyIdentifier(creds) {
    const provider = new VoyageProvider(MOI_NETWORK);
    const wallet = await Wallet.fromMnemonic(creds.mnemonic, creds.derivationPath || DEFAULT_DERIVATION);
    wallet.connect(provider);
...
export async function resolvePeerUrl(agentId, creds) {
    const { registry } = await openRegistry(creds);
    const { found, profile } = await registry.getAgentProfile(agentId);
    if (!found || !profile)
        throw new Error(`MOI agent ${agentId} not found.`);
    ...
    if (!url)
        throw new Error(`MOI agent ${agentId} has no reachable URL.`);
    return url;
}
async function fetchCard(cardUri) {
    if (!cardUri)
        return null;
