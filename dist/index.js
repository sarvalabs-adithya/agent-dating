    ...
    register(api) {
        ...
        registerTool({
            ...
            execute: async () => {
                ...
                if (!id) {
                    try {
                        id = (await getMyCurrentAgentId(creds))?.agentId ?? null;
                    }
                    catch { /* not registered yet */ }
                }
                if (!id)
                    return { ok: false, message: "No registered identity yet — run dating_register first." };
                const viewUrl = await publishViewLink(id, creds.mnemonic);
                if (!viewUrl)
                    return { ok: false, message: "No relay broker configured — the live view runs on the relay." };
