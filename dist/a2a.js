...
export function makeReply(from, text) {
    return { from, text };
}
/**
 ...
 */
export async function sendMessage(peerBaseUrl, from, text, timeoutMs = 8000) {
    ...
    if (looksHtml) {
        throw new Error(`${endpoint} returned an HTML page, not a dating reply. The peer's OpenClaw gateway is reachable, ` +
...
export async function probePeer(peerBaseUrl) {
    ...
    try {
        ...
        if (looksHtml) {
            return {
                ...
                reachable: true,
                servesDating: false,
                detail: "Gateway is up but returned HTML (its Control UI) for the agent-card — the agent-dating plugin's " +
        ...
        try {
            ...
            const dating = card?.skills?.some?.((s) => s?.tags?.includes?.("dating"));
            return {
                url: base,
                reachable: true,
                servesDating: !!dating,
