...
- **Finder-side lines** come from persona, not the initiator's LLM (symmetric brain not yet built).
- **Registration isn't idempotent** → a new MOI id per restart (id churn); ghost ids linger on the relay.
