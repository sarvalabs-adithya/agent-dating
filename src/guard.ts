...
export function repliesSoFar(peerId: string): number {
  return replyCounts.get(peerId) || 0;
}
export function noteReply(peerId: string): void {
  replyCounts.set(peerId, repliesSoFar(peerId) + 1);
}

/** Why to refuse this peer right now, or null to answer normally. */
export function refuseReason(peerId: string): string | null {
  const g = loadGuard();
  if (g.blocked.includes(peerId)) return "blocked";
